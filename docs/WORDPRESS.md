...
The settings screen provides separate post/comment geometry, brush, reaction-cloud, heatmap, cursor, popup, reader-control, and touch-control options. Re:Likes takes page-level priority when both project plugins target the same singular page.

The plugin creates three normalized tables for document revisions, reader subjects, and offset runs. Registered users are authenticated through WordPress and participate in the core privacy exporter and eraser. Anonymous readers use signed random sessions with configurable retention. IP addresses are transformed into expiring transient keys for rate limiting and are not stored with reactions.
